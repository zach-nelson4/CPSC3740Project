...
(define (startEval2 list1 list2 list3)
  ...
  (cond
    ...
    [(and (pair? list3) (pair? list2)) (startEval2 list1 (cadr list2) (cadr list3))]
...
(define (MyLetrecAttributesValues list1)
   (cond [(not(pair? list1)) list1]
        ;[(equal? (length list1) 1) (car(cdr(car list1)))]
        [(pair? list1) (cons (car(cdr(car list1))) (MyLetrecAttributesValues (cdr list1)))]
        ;[(and (pair? list1) (not(equal? (cdr(car list1)) '()))) (cdr(car list1))]
  )
)
